<template>
  <v-container>
        ...
        <indicator-explorer-block
          ...
          :selected-indicator="
            selectedIndicator.length
              ...
              : null
          "
          :indicator-image="getIndicatorImage()"
    ...
    <step-buttons-group
      v-if="selected"
      :continue-title="$t('addIndicatorButton')"
      continue-color="primary"
      continue-class="white"
      @next="save"
      @cancel="
        $router.push({
          name: 'area-list',
    ...
    >
      ...
      <span class="ml-3">{{ $t('indicatorAdded') }}</span>
    </v-snackbar>
...
export default {
  ...
  async asyncData({ $axios, params, query, i18n }) {
    ...
    return {
      ...
      indicatorLevelListSearch: null,
      selectedODS: area.areaIndicatorDTOs.map((i) => i.indicatorUUID),
      ok: null,
      overlay: false,
      search: '',
      customIndicator,
  ...
  computed: {
    availableIndicators() {
      ...
      return Object.keys(this.indicators)
        ...
        .filter((i) => this.$auth.user.productAcronyms.includes(i.id))
    },
